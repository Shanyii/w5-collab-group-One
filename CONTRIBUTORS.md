...
| 角色 | 姓名 | 負責分支 | 任務 | 狀態 |
|------|------|---------|------|------|
| 組長 | 楊姍頤 | `main` | 修改標題 & header 顏色、review 所有 PR | ✔️ |
| 組員 A | 林瑞城 | `feature/member-a` | 新增訊息時間戳 | ✔️ |
| 組員 B | 沈靖恩 | `feature/member-b` | 新增清除對話按鈕 | ✔️ |
| 組員 C | （填名字）| `feature/member-c` | 新增字數統計 | ⬜ |
| 組員 D | 黃柏豪  | `feature/member-d` | 新增深色模式 | ✔️ |
...
### 組長（`main` branch）
- [ v ] 把 `index.html` 裡「第 X 組」改成實際組名
- [ v ] 修改 `style.css` 裡 `header` 的背景顏色
- [ v ] 填入所有組員姓名
- [ v ] 設定 Branch Protection（Settings → Branches → Require PR + **2 approvals**）
- [ v ] Review 每個 PR，留下至少 1 條有意義的 comment
- [ v ] Merge 所有 PR（按順序：A → B → C → D → E，解決 conflict）

### 組員 A（`feature/member-a` branch）
- [ v ] 在 `sendMessage()` 裡，每則訊息加上時間戳（`HH:MM` 格式）
- [ v ] 在 `style.css` 加上 `.timestamp` 樣式（小字、靠右、半透明）
- [ v ]  填寫完整 PR 描述（做了什麼 / 如何測試）
- [ v ] 收到 2 位成員 review 後回應 comment
- [ v ] Review 其他組員的 PR（至少 2 個）

### 組員 B（`feature/member-b` branch）
- [ v ] 在 `index.html` 的 `.input-area` 加入「清除」按鈕
- [ v ] 在 `<script>` 加入 `clearChat()` 函數，清空 `#chat-box`
- [ v ] 在 `style.css` 加上清除按鈕的樣式（與送出按鈕顏色不同）
- [ v ] 填寫完整 PR 描述
- [ v ] 收到 2 位成員 review 後回應 comment
- [ v ] Review 其他組員的 PR（至少 2 個）
